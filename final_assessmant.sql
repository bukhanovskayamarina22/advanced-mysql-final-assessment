...
VALUES (
        ...
        50,
        100
    ), ("P2", "Wood panels", 15, 20, 250), ("P3", "Patio slates", 35, 40, 60), (
        "P4",
        "Sycamore trees ",
        7,
        10,
...
CREATE FUNCTION FINDAVERAGECOST 
	(year VARCHAR(10)) RETURNS INT DETERMINISTIC
	RETURN (
	        SELECT AVG(Cost)
	        FROM Orders
...
DELIMITER //
CREATE PROCEDURE EvaluateProduct(IN id VARCHAR(10), INOUT I2020 INT, INOUT I2021 INT, INOUT I2022 INT)   
BEGIN
    SELECT SUM(Quantity) INTO I2020 FROM Orders WHERE ProductID = id AND YEAR(Date) = '2020';
    SELECT SUM(Quantity) INTO I2021 FROM Orders WHERE ProductID = id AND YEAR(Date) = '2021';
    SELECT SUM(Quantity) INTO I2022 FROM Orders WHERE ProductID = id AND YEAR(Date) = '2022';
END//
...
-- CREATE TRIGGER UpdateAudit AFTER INSERT ON Orders FOR EACH ROW INSERT INTO Audit (OrderDateTime) VALUES (CURRENT_TIMESTAMP());
...
WITH cte_2020 AS (
        SELECT
            CONCAT (SUM(Cost), " (2020)") AS "Total sum of P2 Product"
        FROM Orders
        WHERE
            YEAR (Date) = 2020
            AND ProductID = "P2"),
...
FROM Clients c
    JOIN Activity a ON (
        c.ClientID = a.Properties ->> '$.ClientID'
    );
-- Task 7:
...
-- For example, the output result of GetProfit procedure with the P1 ProductID and Year 2020

DELIMITER //
CREATE PROCEDURE GetProfit (IN id VARCHAR(10), IN year INT) 
BEGIN 
    DECLARE profit DEC(7,2) DEFAULT 0.0;    
    DECLARE q, buy_price, sell_price INT DEFAULT 0;
    SELECT SUM(Quantity) INTO q FROM Orders WHERE ProductID = id AND YEAR(Date) = year;
    SELECT BuyPrice INTO buy_price FROM Products WHERE ProductID = id;
    SELECT SellPrice INTO sell_price FROM Products WHERE ProductID = id;
    SET profit = (sell_price * q) - (buy_price * q);
    SELECT profit;
END //
...
SELECT * FROM DataSummary WHERE YEAR(Date) = 2022 ORDER BY Cost DESC;